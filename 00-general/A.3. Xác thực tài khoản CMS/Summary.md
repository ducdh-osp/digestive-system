...
3. **Bước 3 (BE):**
   ...
   - Nếu hợp lệ: Generate JWT với `Subject` gắn tiền tố `ADMIN:` và thông tin `role`.
4. **Bước 4 (FE):** Nhận Token, lưu riêng vào `adminAccessToken` (tách biệt với Token Customer). Dựa vào `role` trong Token để định tuyến vào đúng khu vực giao diện CMS.
